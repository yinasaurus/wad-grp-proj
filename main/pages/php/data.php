...
if ($action === 'get_business_names_batch' && $method === 'POST') {
    ...
    if ($action === 'all_businesses' && $method === 'GET') {
        ...
        try {
            // Show all businesses that have at least a name (not empty) and a valid email
        ...
        $stmt = $conn->prepare("
            ...
            WHERE b.name IS NOT NULL 
                ...
                AND b.email != ''
                AND (b.user_id IS NULL OR u.user_type = 'business')
            ORDER BY b.name ASC
        ");
    ...
    if ($action === 'update_business_profile' && $method === 'POST') {
        ...
        $userId = $_SESSION['user_id'];
            ...
            if (isset($data['lat']) && $data['lat'] !== '' && $data['lat'] !== null) {
                $lat = (float)$data['lat'];
                if ($lat == 0) $lat = null; // Treat 0 as null
            }
            
            $lng = null;
            if (isset($data['lng']) && $data['lng'] !== '' && $data['lng'] !== null) {
                $lng = (float)$data['lng'];
                if ($lng == 0) $lng = null; // Treat 0 as null
            }
